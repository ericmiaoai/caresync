...
*Last updated: May 28, 2026 — CareSync v1.8.2 (refinements: medical-advice disclaimer footer scoped to `/` and `/scan` via `DISCLAIMER_ROUTES` constant; layout restructured to flex-column with `min-h-full` so the footer sits above the `BottomTabBar`; Privacy Policy link relocated from global footer to Settings → About and Scan AVS contextual caption; login footer and register acceptance line retained as anonymous + signup entry points; Calendar and My Day mobile scroll fix — `PointerSensor` swapped for `MouseSensor` AND sortable wrapper `touchAction` changed from `"none"` to `"manipulation"` so vertical swipes on task cards no longer block the native scroll gesture, long-press still initiates drag-and-drop reorder. My Day was fixed pre-emptively for codebase consistency — the same latent pattern existed there but did not manifest due to layout differences)*
